/*
 * AUTOGENERATED CODE - vendredi 24 novembre 2017, 07:36:42 (UTC+0100)
...
static void jump_0xCB(struct s_gb *s_gb)
{
	/* start of jump_0xCB manual code */

	/* end of jump_0xCB manual code */
}
...
/* call ** [0xCD] : The current pc value plus three is pushed onto the stack, then is loaded with **. */
...
/* exec 0xDD [0xDD] : execute instruction in subtable 0xDD */
static void jump_0xDD(struct s_gb *s_gb)
{
	/* start of jump_0xDD manual code */

	/* end of jump_0xDD manual code */
}

/* sbc a,* [0xDE] : Subtracts * and the carry flag from a. */
...
/* exec 0xED [0xED] : execute instruction in subtable 0xED */
static void jump_0xED(struct s_gb *s_gb)
{
	/* start of jump_0xED manual code */

	/* end of jump_0xED manual code */
}
...
/* exec 0xFD [0xFD] : execute instruction in subtable 0xFD */
static void jump_0xFD(struct s_gb *s_gb)
{
	/* start of jump_0xFD manual code */

	/* end of jump_0xFD manual code */
}
...
const struct s_cpu_z80 instructions_base[] = {
	...
	[0xDD] = {
		.opcode = 0xDD,
		.value = "exec 0xDD",
		.doc = "execute instruction in subtable 0xDD",
		.cycles = 0,
		.size = 1,
		.func = jump_0xDD,
	},
	...
	[0xED] = {
		.opcode = 0xED,
		.value = "exec 0xED",
		.doc = "execute instruction in subtable 0xED",
		.cycles = 0,
		.size = 1,
		.func = jump_0xED,
	},
	...
	[0xFD] = {
		.opcode = 0xFD,
		.value = "exec 0xFD",
		.doc = "execute instruction in subtable 0xFD",
		.cycles = 0,
		.size = 1,
		.func = jump_0xFD,
	},
...
/* sll b [0x30] : The contents of b are shifted left one bit position. The contents of bit 7 are put into the carry flag and a one is put into bit 0. */
static void cb_sll_b(struct s_gb *s_gb)
{
	/* start of cb_sll_b manual code */

	/* end of cb_sll_b manual code */
}

/* sll c [0x31] : The contents of c are shifted left one bit position. The contents of bit 7 are put into the carry flag and a one is put into bit 0. */
static void cb_sll_c(struct s_gb *s_gb)
{
	/* start of cb_sll_c manual code */

	/* end of cb_sll_c manual code */
}

/* sll d [0x32] : The contents of d are shifted left one bit position. The contents of bit 7 are put into the carry flag and a one is put into bit 0. */
static void cb_sll_d(struct s_gb *s_gb)
{
	/* start of cb_sll_d manual code */

	/* end of cb_sll_d manual code */
}

/* sll e [0x33] : The contents of e are shifted left one bit position. The contents of bit 7 are put into the carry flag and a one is put into bit 0. */
static void cb_sll_e(struct s_gb *s_gb)
{
	/* start of cb_sll_e manual code */

	/* end of cb_sll_e manual code */
}

/* sll h [0x34] : The contents of h are shifted left one bit position. The contents of bit 7 are put into the carry flag and a one is put into bit 0. */
static void cb_sll_h(struct s_gb *s_gb)
{
	/* start of cb_sll_h manual code */

	/* end of cb_sll_h manual code */
}

/* sll l [0x35] : The contents of l are shifted left one bit position. The contents of bit 7 are put into the carry flag and a one is put into bit 0. */
static void cb_sll_l(struct s_gb *s_gb)
{
	/* start of cb_sll_l manual code */

	/* end of cb_sll_l manual code */
}

/* sll (hl) [0x36] : The contents of (hl) are shifted left one bit position. The contents of bit 7 are put into the carry flag and a one is put into bit 0. */
static void cb_sll_phl(struct s_gb *s_gb)
{
	/* start of cb_sll_phl manual code */

	/* end of cb_sll_phl manual code */
}

/* sll a [0x37] : The contents of a are shifted left one bit position. The contents of bit 7 are put into the carry flag and a one is put into bit 0. */
static void cb_sll_a(struct s_gb *s_gb)
{
	/* start of cb_sll_a manual code */

	/* end of cb_sll_a manual code */
}
...
const struct s_cpu_z80 instructions_CB[] = {
	...
	[0x30] = {
		.opcode = 0x30,
		.value = "sll b",
		.doc = "The contents of b are shifted left one bit position. The contents of bit 7 are put into the carry flag and a one is put into bit 0.",
		.cycles = 8,
		.size = 2,
		.func = cb_sll_b,
	},
	[0x31] = {
		.opcode = 0x31,
		.value = "sll c",
		.doc = "The contents of c are shifted left one bit position. The contents of bit 7 are put into the carry flag and a one is put into bit 0.",
		.cycles = 8,
		.size = 2,
		.func = cb_sll_c,
	},
	[0x32] = {
		.opcode = 0x32,
		.value = "sll d",
		.doc = "The contents of d are shifted left one bit position. The contents of bit 7 are put into the carry flag and a one is put into bit 0.",
		.cycles = 8,
		.size = 2,
		.func = cb_sll_d,
	},
	[0x33] = {
		.opcode = 0x33,
		.value = "sll e",
		.doc = "The contents of e are shifted left one bit position. The contents of bit 7 are put into the carry flag and a one is put into bit 0.",
		.cycles = 8,
		.size = 2,
		.func = cb_sll_e,
	},
	[0x34] = {
		.opcode = 0x34,
		.value = "sll h",
		.doc = "The contents of h are shifted left one bit position. The contents of bit 7 are put into the carry flag and a one is put into bit 0.",
		.cycles = 8,
		.size = 2,
		.func = cb_sll_h,
	},
	[0x35] = {
		.opcode = 0x35,
		.value = "sll l",
		.doc = "The contents of l are shifted left one bit position. The contents of bit 7 are put into the carry flag and a one is put into bit 0.",
		.cycles = 8,
		.size = 2,
		.func = cb_sll_l,
	},
	[0x36] = {
		.opcode = 0x36,
		.value = "sll (hl)",
		.doc = "The contents of (hl) are shifted left one bit position. The contents of bit 7 are put into the carry flag and a one is put into bit 0.",
		.cycles = 15,
		.size = 2,
		.func = cb_sll_phl,
	},
	[0x37] = {
		.opcode = 0x37,
		.value = "sll a",
		.doc = "The contents of a are shifted left one bit position. The contents of bit 7 are put into the carry flag and a one is put into bit 0.",
		.cycles = 8,
		.size = 2,
		.func = cb_sll_a,
	},
